...
**Важно (useServerStatus):** Пиратские игроки (uuid: null) используют имя как ключ в `allMap` (`p.id || p.uuid || p.name`). Использование только `p.id || p.uuid` давало коллизию ключей — все пиратские игроки маппились на `null` и затирали друг друга между поллами.

## Внешние API
- `api.mcsrvstat.us/3/{ip}` — список онлайн-игроков и статус сервера
- `playerdb.co` — получение UUID по нику
- `crafatar.icehost.xyz/avatars/{uuid}` — аватарки скинов (квадратные с округлёнными углами); используется только для реальных (лицензионных) UUID
- `api.dicebear.com` — резервные аватарки (инициалы); используется для `null` UUID и `offline:*` записей (пиратские игроки)

## Система аватарок

Централизованные утилиты — используются везде вместо инлайн-ternary:

**`backend/utils/avatarUrl.js`** — серверная утилита:
```js
avatarUrl(minecraftUuid, username, size = 64)
// → crafatar если uuid существует и не начинается с 'offline:'
// → dicebear initials иначе
```
Импортируется во все бэкенд-роуты (`posts`, `comments`, `images`, `albums`, `messages`, `logs`, `events`, `news`, `polls`). Все объекты пользователей в ответах API содержат поле `avatarUrl`.

**`src/utils/avatarUrl.js`** — фронтенд утилита:
```js
getAvatarUrl(username, uuid, useFallback = false)
// → crafatar если !useFallback && uuid && !uuid.startsWith('offline:')
// → dicebear иначе
```
Экспортируется из `src/utils/avatarUrl.js`, используется напрямую в компонентах без `usePlayer()`.

**Единый паттерн аватарок во всех компонентах:**
```jsx
<img
  src={avatarUrl || getAvatarUrl(username, null)}
  alt={username}
  onError={(e) => { e.target.onerror = null; e.target.src = getAvatarUrl(username, null); }}
/>
```
- `src` — API-предоставленный URL (может быть crafatar или dicebear с бэкенда)
- Fallback при `null` — dicebear по нику
- `onError` — защита от битых ссылок, fallback на dicebear; `onerror = null` предотвращает бесконечный цикл

**Пиратские игроки (offline)**: UUID хранится как `offline:<name>` в БД. `avatarUrl()` и `getAvatarUrl()` проверяют это и используют dicebear вместо crafatar.
...
| `/dashboard/events/create` | Dashboard/EventCreate | Создание события (любой авторизованный) |
| `/dashboard/events/:slug/edit` | Dashboard/EventCreate | Редактирование события (автор или editor и выше) |
...
| `/court/cases/:id/edit` | Court/CourtCaseCreate | Редактирование заседания (manage_court или admin+) |
| `/court/cases/:id` | Court/CourtCaseDetailPage | Страница заседания с таймером, описанием, приговором |
...
| POST | `/api/auth/login` | все | Вход, возвращает JWT + `{ user: { ..., customRoles, customPermissions } }`; забаненные **могут войти** |
| GET | `/api/auth/me` | JWT (без requireAuth) | Данные текущего пользователя + `customRoles`, `customPermissions`; **не блокирует забаненных** |
| POST | `/api/tickets` | все | Создать заявку на регистрацию |
| GET | `/api/tickets/admin` | admin+ | Список pending-тикетов |
| GET | `/api/tickets/admin/history` | admin+ | Последние 50 обработанных тикетов (approved/rejected) с именем модератора (`approvedByUsername`); параметры `limit`, `offset` |
...
| GET | `/api/players` | все | Все игроки из БД (включает `isBanned`, `banReason`, `customRoles: [{id, name, color}]` — для фильтрации по ролям на Home) |
| POST | `/api/players/sync` | все | Upsert батча онлайн-игроков (не трогает `is_banned`) |
| POST | `/api/players/ban-by-name/:name` | admin+ или `ban_users` | **Универсальный бан** по нику `{ reason? }`; если у игрока настоящий UUID — обновляет `players` + `users`; иначе создаёт/обновляет `offline:<name>` запись |
| POST | `/api/players/unban-by-name/:name` | admin+ или `ban_users` | **Универсальный разбан** по нику; снимает бан в `players` и `users`, очищает `ban_reason` |
...
| POST | `/api/users/:id/ban` | admin+ или `ban_users` | Забанить пользователя `{ reason? }`; нельзя банить равного/высшего |
| POST | `/api/users/:id/unban` | admin+ или `ban_users` | Разбанить пользователя |
| POST | `/api/users/:id/reset-password` | admin+ или `manage_user_accounts` | Сбросить пароль; при следующем логине любой пароль ≥6 символов становится новым; нельзя применять к равному/высшему |
| PUT | `/api/users/:id/username` | admin+ или `manage_user_accounts` | Изменить логин `{ username }`; нельзя применять к равному/высшему |
| POST | `/api/users/:id/clear-data` | admin+ или `manage_user_accounts` | Очистить данные аккаунта; тело: `{ sections?: ['posts','images','profile','chats'] }` — по умолчанию все 4; файлы удаляются с диска; сам аккаунт остаётся; нельзя применять к равному/высшему |
| DELETE | `/api/users/:id` | admin+ или `manage_user_accounts` | Полностью удалить аккаунт: сначала удаляет файлы с диска, затем запись из `users` (CASCADE чистит посты, вложения, изображения, альбомы, лайки, комментарии, диалоги); нельзя удалять равного/высшего |
...
| GET | `/api/conversations` | JWT | Список диалогов текущего пользователя; объект `partner` включает `avatarUrl` |
| GET | `/api/conversations/unread-count` | JWT | Число непрочитанных сообщений |
| GET | `/api/conversations/:userId/messages` | JWT | История сообщений с пользователем (не создаёт диалог); объекты `sender` включают `avatarUrl` |
...
| POST | `/api/news` | editor+ или `manage_news` | Создать новость |
| PUT | `/api/news/:slug` | editor+ или `manage_news` | Обновить новость |
| DELETE | `/api/news/:slug` | admin+ | Удалить новость |
| POST | `/api/news/upload-image` | editor+ или `manage_news` | Загрузить изображение/видео в редактор |
...
| POST | `/api/events` | JWT | Создать событие; обязательно `start_time` (unix timestamp) |
| PUT | `/api/events/:slug` | JWT (автор) или editor+ или `manage_events` | Обновить событие |
| DELETE | `/api/events/:slug` | JWT (автор) или admin+ или `manage_events` | Удалить событие |
| POST | `/api/events/upload-image` | JWT | Загрузить изображение/видео в редактор события |
...
| GET | `/api/roles` | все | Список кастомных ролей (по приоритету); возвращает `{ id, name, color, permissions[], priority }` |
| POST | `/api/roles` | admin+ или `manage_custom_roles` | Создать роль `{ name, color, permissions[] }` |
| PUT | `/api/roles/:id` | admin+ или `manage_custom_roles` | Обновить роль (name, color, permissions) |
| DELETE | `/api/roles/:id` | admin+ или `manage_custom_roles` | Удалить роль; CASCADE удаляет `user_custom_roles` |
| PUT | `/api/roles/:id/move` | admin+ или `manage_custom_roles` | Сменить приоритет `{ direction: 'up'\|'down' }` — меняет местами соседние приоритеты |
| GET | `/api/roles/system/:role/users` | все | Пользователи с системной ролью (user/editor/admin/creator) |
| GET | `/api/roles/:id/users` | все | Пользователи с данной кастомной ролью |
| POST | `/api/roles/:id/users` | admin+ или `assign_custom_roles`/`manage_custom_roles` | Назначить роль `{ userId }` |
| DELETE | `/api/roles/:id/users/:userId` | admin+ или `assign_custom_roles`/`manage_custom_roles` | Отозвать роль у пользователя |
| POST | `/api/court/tickets` | JWT | Создать жалобу `{ accusedName, title, description, attachments? }` |
| GET | `/api/court/tickets/my` | JWT | Мои жалобы |
| GET | `/api/court/tickets/pending-count` | manage_court или admin+ | Кол-во pending тикетов (для бейджа в Sidebar) |
| GET | `/api/court/tickets` | manage_court или admin+ | Все тикеты (фильтр `?status=`) |
| GET | `/api/court/tickets/:id` | JWT (создатель или manage_court+) | Один тикет + сообщения чата |
| POST | `/api/court/tickets/:id/review` | manage_court или admin+ | Взять тикет в работу → статус `reviewing` |
| POST | `/api/court/tickets/:id/reject` | manage_court или admin+ | Отклонить тикет `{ reason? }` |
| POST | `/api/court/tickets/:id/close` | manage_court или admin+ | Закрыть тикет → статус `closed` |
| POST | `/api/court/tickets/:id/messages` | JWT (создатель или manage_court+) | Сообщение в чате тикета `{ content, fileUrl?, fileType?, fileName?, filesJson? }` |
| GET | `/api/court/cases` | JWT | Список судебных заседаний |
| POST | `/api/court/cases` | manage_court или admin+ | Создать заседание `{ title, description?, verdict?, ticketId?, hearingAt?, status? }` |
| GET | `/api/court/cases/:id` | JWT | Одно заседание |
| PUT | `/api/court/cases/:id` | manage_court или admin+ | Обновить заседание |
| DELETE | `/api/court/cases/:id` | manage_court или admin+ | Удалить заседание |
...
6. На странице `/dashboard/tickets` одобряет/отклоняет заявки — **две вкладки**:
   - **Ожидающие** — pending-тикеты с кнопками «Подтвердить» / «Отклонить»; счётчик в красном бейдже на вкладке
   - **История** — обработанные тикеты (approved/rejected) с бейджем статуса, именем модератора и датой решения; загружается при первом открытии вкладки, сбрасывается после каждого действия
7. При одобрении → автоматически создаётся пользователь в `users`, статус меняется на `approved`; `approved_by` = id администратора, `approved_at` = unix timestamp
...
5. **Статус** — сворачиваемая группа (как UI-группы); внутри: поле ввода (до 50 символов, `bio` в БД) + стиль (цвет текста, размер шрифта, жирность) + кнопка «Сбросить всё»
6. Секция «Настройка UI-элементов» — 3 раскрывающихся группы: **Шапка профиля**, **Область контента**, **Карточки и вкладки**; каждая: фон (цвет + прозрачность + blur), рамка (цвет + ширина + радиус), текст и акцент + кнопка «Сбросить всё»
   - «Шапка профиля» и «Карточки и вкладки» имеют цвет текста + акцент; дефолтная жирность **700**
...
## Middleware
- `requireAuth` — проверяет JWT токен из заголовка `Authorization: Bearer <token>`; загружает `req.user.customPermissions` (Set) из `user_custom_roles + custom_roles`; забаненные пользователи получают 403 на все write-действия
...
- `isAdminOrPerm(perm)` — фабрика middleware: пропускает если `level ≥ 3` ИЛИ `customPermissions.has(perm)`
- `isEditorOrPerm(perm)` — фабрика middleware: пропускает если `level ≥ 2` ИЛИ `customPermissions.has(perm)`
- `optionalAuth` — если токен есть и валиден → req.user (БЕЗ загрузки customPermissions), иначе null; для забаненных всегда null
...
Правила управления системными ролями:
...
- Забаненный пользователь: **может войти** и просматривать сайт (read-only), но `requireAuth` блокирует все write-действия с кодом 403; `/api/auth/me` работает без `requireAuth`

### Кастомные роли и права (`custom_roles`)
Поверх системных ролей существует система кастомных ролей. Каждая роль имеет цвет, приоритет и набор прав (`permissions[]`). Один пользователь может иметь несколько кастомных ролей одновременно.

**Список прав (permission ids):**
| ID | Описание | Группа |
|----|----------|--------|
| `manage_news` | Редактировать новости и события (создание/удаление) | Контент |
| `manage_events` | Редактировать события | Контент |
| `ban_users` | Банить/разбанить игроков | Модерация |
| `manage_tickets` | Управлять заявками на регистрацию | Модерация |
| `moderate_content` | Удалять чужой контент (посты/комментарии/медиа/фото) | Модерация |
| `assign_custom_roles` | Назначать кастомные роли игрокам | Администрирование |
| `manage_user_accounts` | Управлять аккаунтами (сброс пароля, смена логина, очистка данных, удаление) | Администрирование |
| `view_logs` | Просматривать логи активности | Администрирование |
| `manage_custom_roles` | Управлять кастомными ролями (CRUD + назначение) | Администрирование |

**Ограничения `manage_user_accounts`**: может управлять только пользователями с ролью ниже `admin` (level < 3).

**Ограничения `ban_users`**: пользователь без системного admin может банить только тех, чья системная роль < admin.

**Паттерн проверки на фронте:**
```js
const perms = user?.customPermissions ?? [];   // string[]
const canDo = callerLevel >= 3 || perms.includes('perm_id');
```

**Паттерн проверки на бэкенде:**
```js
const canDo = callerLevel >= ROLE_LEVEL.admin || req.user.customPermissions?.has('perm_id');
// req.user.customPermissions — это Set (загружается в requireAuth)
```

**`customPermissions` в объекте пользователя:**
- Бэкенд в `requireAuth`: `req.user.customPermissions` — **Set** строк
- `POST /api/auth/login` и `GET /api/auth/me` возвращают: `customPermissions` — **массив** строк
- `AuthContext` хранит как массив; компоненты используют `.includes()`
- **Важно:** `optionalAuth` НЕ загружает `customPermissions` — только `requireAuth`
...
- Поток: загрузка через `POST /api/upload` (поле `files[]`) → сохранение через `POST /api/images`; прогресс отображается через `onUploadProgress` как `Загрузка N%...`, затем `Сохранение...`; то же поведение реализовано в формах вкладки «Фото» и «Альбомы» в PlayerPage
...
- Кнопка **«Удалить»** — отображается на чужом профиле для admin/creator **или** `manage_user_accounts` (только если уровень/роль цели ниже admin или ниже текущего); `canDeleteProfile = canManageAccounts && profileLevel < callerLevel || (manage_user_accounts && profileLevel < 3)`; находится в `.player-page__top-actions` справа рядом с «Написать»; при клике открывается дропдаун-меню через `createPortal` в `document.body` с `position:fixed`:
...
- Пункты (по порядку): Главная, Лента, Галерея, Новости, События, **Суд** (все пользователи), Сообщения (только авторизованные), Заявки (admin+ или `manage_tickets`), Роли (admin+ или `manage_custom_roles`/`assign_custom_roles`), Логи (admin+ или `view_logs`)
- **Бейджи уведомлений** — красная обводка (`sidebar__item--pending`) + числовой счётчик (`sidebar__item-badge`):
  - **Новости** — счётчик новых новостей, опубликованных после последнего посещения `/news`; хранится в `localStorage('sosaland:lastSeenNews')`; опрос раз в 60 сек; сбрасывается мгновенно при переходе на `/news`
  - **События** — счётчик новых событий, созданных после последнего посещения `/events`; хранится в `localStorage('sosaland:lastSeenEvents')`; опрос раз в 60 сек; сбрасывается мгновенно при переходе на `/events`
  - **Сообщения** — непрочитанные, опрашиваются каждые 15 секунд (`/api/conversations/unread-count`); сбрасывается мгновенно при переходе на `/messages`
  - **Заявки** — pending-тикеты на регистрацию; опрос раз в 60 сек; сбрасывается мгновенно при переходе на `/dashboard/tickets`
  - **Суд** — pending тикеты суда; только для manage_court/admin+; опрос раз в 60 сек (`/api/court/tickets/pending-count`)
- При первом посещении (нет `localStorage`) для новостей/событий — отметка ставится сразу, бейдж не показывается (чтобы не спамить при первом заходе)
- Непрочитанные сообщения опрашиваются каждые 15 секунд (`/api/conversations/unread-count`)
- Блок пользователя наверху: аватарка + имя → ссылка на профиль; под именем — **кастомная роль с наивысшим приоритетом** (наименьший `priority`) в цвете роли; если кастомных ролей нет — системная роль:
...
Страница `/dashboard/logs` — для `admin`/`creator` или пользователей с правом `view_logs`. Фиксирует ключевые действия пользователей на сайте.
...
- **`PollBuilder`** (`src/Components/PollBuilder/`) — конструктор: вопрос, описание, варианты ответа (2–25), настройки. Вызывается из RichTextEditor, PostForm и PollViewer через portal-модал. Props: `onConfirm(pollData)`, `onCancel()`, `initialData?`, `onDelete?` — если передан `initialData`, переходит в режим редактирования (заголовок «✏️ Редактировать опрос», кнопка «Сохранить»). В режиме редактирования `options` в `pollData` — массив `[{id?, text}]`; в режиме создания — массив строк. Варианты с `votesCount > 0` нельзя удалить (кнопка удаления задизейблена с подсказкой). **Оверлей не закрывается по клику вне окна** — только через кнопку ✕. Если передан `onDelete` и `isEdit` — в футере слева появляется кнопка «Удалить опрос» (красная обводка, `margin-right: auto`).
- **`PollViewer`** (`src/Components/PollViewer/`) — отображение опроса: голосование (radio/checkbox), прогресс-бары результатов, пагинация вариантов (по 5), кнопка «Добавить вариант», «Изменить ответ». Цвета через `--cards-*` CSS-переменные. Кнопка **✏️** в шапке видна **только автору контента** (`user.id === poll.authorId`) — бэкенд возвращает `authorId` из таблицы `posts.user_id` или `news.author_id`; открывает PollBuilder в режиме редактирования через portal; сохранение вызывает `PUT /api/polls/:id` с обновлёнными полями и вариантами. Props: `pollId`, `cssVars?`, `onDeleted?` — вызывается после успешного удаления опроса. **Удаление**: PollBuilder передаётся `onDelete={handleDelete}` — `handleDelete` показывает `showConfirm`, вызывает `DELETE /api/polls/:id`, устанавливает внутренний `deleted=true` (компонент возвращает `null`) и вызывает `onDeleted?.()` у родителя.
...
- `status` TEXT DEFAULT 'scheduled' (migration 040) — `'scheduled'` | `'in_progress'` | `'completed'`; явный статус переопределяет таймер на основе времени в `EventCard` и `EventDetailPage`
...
**custom_roles:**
- `id` TEXT PK (UUID)
- `name` TEXT NOT NULL — название роли (отображается на профиле и в сайдбаре)
- `color` TEXT NOT NULL DEFAULT '#4aff9e' — цвет бейджа роли
- `permissions` TEXT NOT NULL DEFAULT '[]' — JSON-массив строк с ID прав
- `priority` INTEGER — чем меньше, тем выше в иерархии; роль с наименьшим priority отображается первой
...
- `id` TEXT PK (UUID)
- `user_id` TEXT NOT NULL → users.id ON DELETE CASCADE
- `role_id` TEXT NOT NULL → custom_roles.id ON DELETE CASCADE
- `granted_by` TEXT → users.id ON DELETE SET NULL
- `granted_at` INTEGER DEFAULT unix_now()
- UNIQUE(user_id, role_id) — один игрок не может иметь одну роль дважды
- INDEX on user_id, INDEX on role_id
...
**court_tickets** (migration 038):
- `id` TEXT PK; `created_by` → users.id CASCADE; `accused_name` TEXT NOT NULL; `title` TEXT NOT NULL; `description` TEXT NOT NULL
- `status` TEXT DEFAULT 'pending' — `'pending'` | `'reviewing'` | `'closed'` | `'rejected'`
- `attachments` TEXT (migration 039) — JSON-массив `[{fileUrl, fileType, fileName}]` с прикреплёнными доказательствами
- `reviewer_id` → users.id ON DELETE SET NULL; `review_started_at` INTEGER; `closed_at` INTEGER; `closed_by` → users.id; `rejection_reason` TEXT
- `created_at`, `updated_at` INTEGER

**court_ticket_messages** (migration 038 + 039):
- `id` TEXT PK; `ticket_id` → court_tickets.id CASCADE; `sender_id` → users.id ON DELETE SET NULL
- `content` TEXT NOT NULL; `is_system` INTEGER DEFAULT 0 — 1 = системное сообщение (не пользователь)
- `file_url`, `file_type`, `file_name` TEXT (migration 039) — первый файл сообщения
- `files_json` TEXT (migration 039) — JSON-массив доп. файлов (2+)
- `created_at` INTEGER; INDEX on ticket_id

**court_cases** (migration 038 + 041 + 042):
- `id` TEXT PK; `ticket_id` → court_tickets.id ON DELETE SET NULL (опционально)
- `title` TEXT NOT NULL (≤200); `description` TEXT (HTML из RichTextEditor); `verdict` TEXT (HTML из RichTextEditor)
- `hearing_at` INTEGER — unix timestamp даты заседания
- `preview_image_url` TEXT (migration 041) — шапка вкладки «Описание»
- `preview_verdict_image_url` TEXT (migration 042) — шапка вкладки «Итог/Приговор»
- `created_by` → users.id CASCADE; `status` TEXT DEFAULT 'scheduled' — `'scheduled'` | `'in_progress'` | `'completed'`
- `created_at`, `updated_at` INTEGER; INDEX on ticket_id

## Система суда

Страница `/court` — три вкладки. Вкладка «Суды» открыта для всех (включая неавторизованных); вкладки «Подать жалобу» и «Тикеты/Мои жалобы» — только для авторизованных (показывается заглушка с кнопкой «Войти»).

### Вкладка «⚖️ Суды»
- Сетка судебных заседаний (дизайн как `EventsPage`, 3 колонки)
- `CourtCaseCard` — карточка с таймером/статусом, названием, именем обвиняемого; **3 состояния** (те же что у событий):
  - Дата в будущем → «Через X д Y ч Z мин» (зелёный)
  - `in_progress` или дата прошла → «Идёт» (оранжевый, `event-timer--in-progress`)
  - `completed` → «Завершено» (серый, `event-timer--completed`)
  - Нет даты — бейдж не отображается
  - Шапка: `completed` + задана `previewVerdictImageUrl` → показывает её, иначе `previewImageUrl`
- `CourtCaseCreate` (`/court/cases/create` и `/court/cases/:id/edit`) — форма: две вкладки (Описание / Итог), у каждой вкладки своя шапка (`previewImageUrl` / `previewVerdictImageUrl`), RichTextEditor, дата заседания, статус, привязка к тикету; если дата уже прошла и статус `scheduled` — автоматически переключается на `in_progress`
- `CourtCaseDetailPage` (`/court/cases/:id`) — те же 3 состояния и цвета (классы `event-detail__timer--in-progress` / `event-detail__timer--completed`); шапка зависит от активной вкладки; при загрузке если `status === 'completed'` и есть приговор — открывается вкладка «Итог/Приговор» по умолчанию; кнопки редактировать/удалить для manage_court+; контент парсится через `CaseContent` (те же маркеры что у событий/новостей)
- Кнопка «+ Создать заседание» — manage_court или admin+; ведёт на `/court/cases/create`

### Вкладка «📋 Подать жалобу»
- Только для авторизованных; неавторизованным показывается заглушка с кнопкой «Войти»
- Форма: ник обвиняемого (с @-автодополнением из списка игроков), тема, описание до 3000 символов (с @-автодополнением), прикрепить файлы-доказательства
- Файлы загружаются через `POST /api/upload` → сохраняются в поле `attachments` тикета
- После отправки показывается сообщение об успехе

### Вкладка «🎫 Тикеты» / «Мои жалобы»
- Только для авторизованных; неавторизованным показывается заглушка с кнопкой «Войти»
- Для manage_court/admin+: все тикеты с фильтрами по статусу; кнопки «Рассмотреть» / «Отклонить» / «Закрыть»
- Для обычных: только свои жалобы (`GET /api/court/tickets/my`)
- Двухколоночный макет: список слева, детали + чат справа; на мобилке (<700px) — переключаемые виды: список / детали (кнопка «← Назад к списку»)
- **Чат тикета**: `MessageInput` + рендеринг сообщений в стиле `ChatWindow`; системные сообщения как разделители
  - Создатель тикета может писать всегда (пока статус не `closed`/`rejected`)
  - Модератор (manage_court) может писать **только после того как взял тикет в работу** (`статус = 'reviewing'`); до этого показывается плашка «⏳ Примите тикет в работу, чтобы начать общение»
- Файлы в чате поддерживают медиа-сетку, аудио, документы + лайтбокс через `ImageModal`
- На мобилке чат занимает `height: 65vh`, страница не скроллируется (overflow: hidden на `.court-page--tickets`)

### Sidebar
- Ссылка «⚖️ Суд» видна **всем пользователям** (включая неавторизованных); расположена между «События» и «Сообщения»
- Красный числовой бейдж с количеством pending тикетов — только для manage_court/admin+; опрашивается раз в 60 сек

### Право `manage_court`
- Входит в группу «Модерация» в `src/utils/permissions.js` и `backend/utils/permissions.js`
- Даёт доступ к рассмотрению тикетов и управлению заседаниями
...
- **Опрос** (кнопка 📊): создаётся через `onCreatePoll`; редактирование и удаление — через кнопку ✏️ в `PollViewer` (не из редактора). **Удаление маркера из редактора**: клик на плашку `rte-poll-marker` → `showConfirm('Удалить опрос из статьи?')` → `marker.remove()` + `handleInput()`; при сохранении новости/события бэкенд удаляет orphaned-опросы через `deleteOrphanedPolls()`. Все маркеры (`.rte-slider`, `.rte-player-list`, `.rte-image-row`, `.rte-poll-marker`) имеют `cursor: pointer` в SCSS
...
- Таймер обновляется каждые 60 секунд через `setInterval`; логика `getTimerLabel(startTime, endTime, status)` — **3 состояния**:
  - `delta > 0` (ещё не началось) → «Через X д Y ч Z мин» (зелёный, класс по умолчанию)
  - `in_progress` или время прошло → «Идёт» (оранжевый, класс `event-timer--in-progress`)
  - `completed` или `now > endTime` → «Завершено» (серый, класс `event-timer--completed`)
- **Шапка карточки**: если `status === 'completed'` и задана `previewImageResultsUrl` — показывается шапка итогов, иначе основная
- Кнопка «+ Создать событие» — для всех авторизованных пользователей; ссылка на `/dashboard/events/create`
...
- Заголовок и таймер (`EventTimer`) в одной строке `.event-detail__title-row`; те же 3 состояния и цвета, что в карточке (классы `event-detail__timer--in-progress` / `event-detail__timer--completed`)
- **Две вкладки**: «Основная информация» (всегда) и «Итоги» (скрыта если `contentResults` пуст); при загрузке если `status === 'completed'` и есть итоги — открывается вкладка «Итоги» по умолчанию
...
- Кнопка «✏️ Редактировать» — автору события или editor+/`manage_events`; кнопка «🗑 Удалить» — автору или admin+/`manage_events`
...
- Создание — любой авторизованный; редактирование — автор события или editor+/`manage_events`; компонент `EventCreate.jsx`
...
## Управление кастомными ролями (RolesPage)

Страница `/dashboard/roles` — доступна admin/creator **или** пользователям с правом `manage_custom_roles`/`assign_custom_roles`.

**Два уровня доступа:**
- `manage_custom_roles` — полный CRUD: создать, переименовать, сменить цвет/права, удалить, назначать игрокам
- `assign_custom_roles` — только назначение/отзыв существующих ролей игрокам (без CRUD)

**Интерфейс:**
- Список ролей с цветными бейджами; кнопки ▲/▼ для смены приоритета (только manage_custom_roles)
- Клик на роль → список пользователей с этой ролью + кнопка «Удалить» у каждого
- Кнопка «+ Добавить» → форма: имя, цвет (colorpicker), набор прав (чекбоксы по группам)
- Раздел «Системные роли» — просмотр кто имеет какую системную роль (user/editor/admin/creator)

**`PlayerPage` — блок кастомных ролей:**
- Цветные бейджи кастомных ролей игрока под именем в шапке профиля
- Кнопка «+» для назначения роли, «×» у каждого бейджа для отзыва
- Блок виден если `customRoles.length > 0` ИЛИ у текущего пользователя есть `canAssignRoles`
- `canAssignRoles = callerLevel >= 3 || perms.includes('assign_custom_roles') || perms.includes('manage_custom_roles')`

**`Sidebar` — бейдж роли:**
- Под именем пользователя в сайдбаре отображается цветной бейдж **кастомной роли с наивысшим приоритетом** (наименьший `priority`)
- Если кастомных ролей нет — показывается системная роль с цветом: creator → `#ffd700`, admin → `#4aff9e`, editor → `#7eb8f7`, user → `#555`

## Управление игроками (Home — карточки)

На главной странице `/` каждая карточка игрока (`PlayerCard`) имеет кнопку `⋮` в правом верхнем углу — видна пользователям с правом `ban_users` ИЛИ `manage_user_accounts` ИЛИ системным admin/creator.
...
```js
const canBan      = callerLevel >= 3 || perms.includes('ban_users');
const canAccounts = callerLevel >= 3 || perms.includes('manage_user_accounts');
const canManage   = canBan || canAccounts;  // показывает ⋮
```

Пункты меню:
- **Забанить / Разбанить** (`canBan = true`) — при бане открывается `showPrompt` для ввода причины; пустая строка = бан без причины, `null` = отмена; кнопка задизейблена если нельзя банить цель
- **🔑 Сбросить пароль** и **📝 Изменить логин** — показываются если `canActOnRole`:
  - `canActOnRole = canAccounts && profile && profile.id !== currentUser.id && (callerLevel > targetLevel || (manage_user_accounts && targetLevel < admin))`

**Правила бана:**
- `ban_users` без системного admin: может банить только тех, чья роль < admin
- Нельзя банить самого себя

Кнопки смены системной роли (admin/editor/user) показываются только если у игрока есть аккаунт (`profile !== null`) и его роль ниже роли текущего пользователя (`targetLevel < callerLevel`). Нельзя управлять собственной ролью.
...
`PlayerCard` принимает пропы: `username`, `status` (`'online'|'offline'`), `currentUser` (объект из `useAuth()`, должен иметь `customPermissions: string[]`), `token`
...
- Аватарки Minecraft везде квадратные с `border-radius: 4–6px` (не круглые); паттерн: `src={avatarUrl || getAvatarUrl(username, null)}` + `onError` fallback на dicebear (см. раздел «Система аватарок»)
